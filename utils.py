...
def random_crop_time(img, mask, patch_size, pos_prob=None): 
    h, w = img.shape
    if min(h, w) < patch_size:
        img = np.pad(img, ((0, max(h, patch_size)-h),(0, max(w, patch_size)-w)), mode='constant')
        mask = np.pad(mask, ((0, max(h, patch_size)-h),(0, max(w, patch_size)-w)), mode='constant')
    ...
    return img_patch, mask_patch,h_start,w_start

def crop(img, mask, patch_size, h_start,w_start): 
    h, w = img.shape
    if min(h, w) < patch_size:
        img = np.pad(img, ((0, max(h, patch_size)-h),(0, max(w, patch_size)-w)), mode='constant')
        mask = np.pad(mask, ((0, max(h, patch_size)-h),(0, max(w, patch_size)-w)), mode='constant')
...
def get_img_norm_cfg(dataset_name, dataset_dir):
    ...
    if  dataset_name == 'NUAA-SIRST':
        ...
        img_norm_cfg={'mean':104.83120727539062, 'std': 56.96275329589844}
    elif dataset_name=="NUDTMIRSDT":
        img_norm_cfg={'mean': 105.98960876464844, 'std': 25.601680755615234}

    else:
        with open(dataset_dir  +'train_' + dataset_name + '.txt', 'r') as f:
            train_list = f.read().splitlines()
        with open(dataset_dir  +'test_' + dataset_name + '.txt', 'r') as f:
        ...
        for img_pth in img_list:
            try:
                img = Image.open((img_dir + img_pth).replace('//', '/')).convert('I').resize((512,512), resample=Image.BILINEAR) # + '.png'BILINEAR NEAREST
            except:
                try:
                    img = Image.open((img_dir + img_pth).replace('//', '/') ).convert('I').resize((512,512), resample=Image.BILINEAR) #+ '.jpg'
                except:
                    # print('img_pth',img_pth)
                    # pdb.set_trace()
                    img = Image.open((img_dir + img_pth).replace('//', '/') ).convert('I').resize((512,512), resample=Image.BILINEAR) #+ '.bmp'
...
def pad_image_to_max_dimensions(img, max_height, max_width):
    """将图像填充到最大高度和宽度"""
    # pdb.set_trace()
    ...
    # 使用零填充将图像调整为最大尺寸
    padded_img = np.pad(np.array(img), ((0, pad_h), (0, pad_w)), mode='constant', constant_values=0) #padded_img = np.pad(np.array(img), ((0, pad_h), (0, pad_w), (0, 0)), mode='constant', constant_values=0)
