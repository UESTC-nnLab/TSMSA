...
class SSTUNet(nn.Module):
    def __init__(self, in_channels=1, layers=[3,3,3], channels=[8,16,32,64], fuse_mode='AsymBi', tiny=False, classes=1,
        ...
        if tiny:  # 默认是False
            ...
            #feature_embedding(in_channels=in_channels,stem_width=stem_width,norm_layer=norm_layer,t_frame=num_frame)
            self.backbone = feature_embedding(in_channels=in_channels,layers=layers, channels=channels,norm_layer=norm_layer,t_frame=num_frame)
            # nn.Sequential(
            # # self.stem.add(nn.Conv2D(channels=stem_width*2, kernel_size=3, strides=2,
            # #                          padding=1, use_bias=False))
            # # self.stem.add(norm_layer(in_channels=stem_width*2))
            ...
            # nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
            # ).cuda()
        self.SSTCell=SSTNet(input_dim=16*4, hidden_dim=[16*4], kernel_size=(3, 3), num_slices=1, num_layers=1)
        self.fusion1 = nn.Sequential(
                nn.Conv2d(80*4, 80*4, kernel_size=3, stride=1, padding=1, bias=False, groups=80), #kernel_size=5, stride=4, padding=1,
                nn.LeakyReLU(),
                nn.Conv2d(80*4, 16*4, kernel_size=1, stride=1, padding=0, bias=False),
                nn.LeakyReLU())
        self.fusion2 = nn.Sequential(
                nn.Conv2d(32*4, 32*4, kernel_size=3, stride=1, padding=1, bias=False, groups=32), #kernel_size=5, stride=4, padding=1,
                nn.LeakyReLU(),
                nn.Conv2d(32*4, 16*4, kernel_size=1, stride=1, padding=0, bias=False),
                nn.LeakyReLU()) 
        self.deconv2=nn.ConvTranspose2d(in_channels=channels[3], out_channels=channels[2], kernel_size=(4, 4),stride=2, padding=1)
        self.deconv1=nn.ConvTranspose2d(in_channels=channels[2], out_channels=channels[1], kernel_size=(4, 4),stride=2, padding=1)
        self.fuse2 = AsymBiChaFuse(channels=channels[2])
        self.fuse1 = AsymBiChaFuse(channels=channels[1])
    ...
    def forward(self,  x):
        ...
        B1,T1,C1,H1,W1=feat_S.shape
        feat_SF=feat_S.view(B1,T1*C1,H1,W1)
        # feat_SF=feat_S.view(B, -1, hei//16, wid//4)
        ...
        # pdb.set_trace()
        out = self.fuse2(out, multi_key[1]) # [8,16,192,248]
        
        out = self.deconv1(out) #[8,8,192*2,248*2]
